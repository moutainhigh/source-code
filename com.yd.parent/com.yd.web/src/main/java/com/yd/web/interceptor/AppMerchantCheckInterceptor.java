...
@Component
public class AppMerchantCheckInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        // 方法上的 权限控制
        AppMerchantCheck methodAuth = handlerMethod.getMethodAnnotation(AppMerchantCheck.class);
        // 类上的权限控制
        AppMerchantCheck clzAuth = handlerMethod.getMethod().getDeclaringClass().getAnnotation(AppMerchantCheck.class);
        // 是否是ajax
        ...
        if (methodAuth != null || clzAuth!=null) {
            ...
            System.out.println("merchantId = " + merchantId + "sessionId = " + sessionId);
            String url = request.getRequestURI();
            System.out.println("请求全路径url = " + url);
            ...
            boolean isLogin = ydLoginService.checkUserLogin(YdLoginUserSourceEnums.YD_APP_MERCHANT, merchantId, sessionId);
            if (!isLogin) {
                processNotLogin(url, response, request,handlerMethod);
                return false;
            }
     ...
     * @return
     */
    private boolean checkPermission(Integer merchantId, String url, AppMerchantCheck methodAuth, AppMerchantCheck clzAuth) {
