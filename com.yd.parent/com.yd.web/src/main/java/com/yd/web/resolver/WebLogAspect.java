...
public class WebLogAspect {
    ...
    @Reference
    private YdVisitorLogService ydVisitorLogService;
     ...
     */
    @Pointcut("execution(public * com.yd.web.controller.front..*.*(..))")
    public void webLog() {
    }

    private static final String[] IGNORE_LIST = new String[]{
            "com.yd.web.controller.IndexPageController.index"
    };
...
//        }
    ...
    private void setVisitorLog(VisitInfo visitInfo) {
        ...
        try {
            ...
            YdMerchantItemResult ydMerchantItemResult = ydMerchantItemService.getYdShopMerchantItemById(merchantItemId);
            if (ydMerchantItemResult == null) return;

            YdItemResult ydItemResult = ydItemService.getYdItemById(ydMerchantItemResult.getItemId());
            if (ydItemResult == null) return;

            YdVisitorLogResult ydVisitorLogResult = new YdVisitorLogResult();
            ydVisitorLogResult.setUserId(Integer.valueOf(visitInfo.getVisitorUserId()));
            ydVisitorLogResult.setMerchantId(ydMerchantItemResult.getMerchantId());
            ydVisitorLogResult.setItemId(ydItemResult.getId());
            ydVisitorLogResult.setMerchantItemId(merchantItemId);
            logger.info("====setVisitorLog的入参=" + JSON.toJSONString(ydVisitorLogResult, true));
            ydVisitorLogService.insertYdVisitorLog(ydVisitorLogResult);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
